...
`symcon.icon.presets.build_jw` preset) against a reference trajectory produced by the
...
1. `uv run python validation/L4_idealized/make_reference.py` — generates and caches
   ...
   item 4) under `~/.cache/symcon/l4_reference/` (override:
...
2. `uv run pytest validation/L4_idealized/test_jw.py -m "data and slow"` — runs the
   9-day symcon trajectory (also hours) and compares:
