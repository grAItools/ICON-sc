...
**Horizon:** vertical slice fully specified — SCM physics column (satad + graupel, SUS coupling, F-tier gradients) and idealized dycore (NonhydroSolver + diffusion + Jablonowski–Williamson, T1 plan) — with post-slice phases outlined in `development/work/proposals/`.
...
## 5. Post-slice phases (outlined only — see `development/work/proposals/`)
