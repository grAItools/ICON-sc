...
5. L8 harness in `symcon/core/testing`: `taylor_test(f, x, v)`, `dot_product_test(f, x, v, w)` — generic, reused in P6.
6. Example + validation seeding; fp64 via `jax.config.update("jax_enable_x64", True)` in test conftest.
**Pitfalls:** carry ordering must be deterministic (sort leaves by name at generation); mask arrays must be step-indexed `xs`, not Python ints, or scan retraces; keep the functional path free of any `symcon.core.state.dataarray` import (trace purity).
