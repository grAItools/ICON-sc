...
Scope: the verification gate every step/task must leave green, its baseline counts, and
the rules for reading gate output. Keep-current rule: a merged task that changes any
count updates this file in the same commit.
