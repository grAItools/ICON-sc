...
1. Pick a work unit whose dependencies are merged (the 001–014 slice DAG:
   `development/work/reports/report-0000-overview.md` §2). Branch: `work/NNN-<slug>`.
2. Read the spec (`development/specs/NNN_<slug>_spec.md`) fully, then the plan
   (`development/plans/NNN_<slug>_plan.md`). The spec's *Frozen interfaces* are
...
4. Implement. Tests alongside. Tolerances stated in specs are contracts: loosening one
   requires a written justification in the work unit's record and human sign-off in
   the PR.
5. Gate before PR — the full battery and baselines in
   `development/policies/verification_gates.md`.
6. Write the record `development/records/NNN_<slug>_record/` (STATUS template in
   `development/policies/document_kinds.md`): what was built, deviations + why,
   follow-ups, benchmark/plot artifacts if the spec asks. One PR per work unit; fill
   the PR template.
