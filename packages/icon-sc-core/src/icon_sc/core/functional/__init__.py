...
``icon_sc.core`` — importing :mod:`icon_sc.core` itself never imports jax):

- :mod:`icon_sc.core.functional.pytree` — generated frozen-dataclass PyTrees:
  ``StateTree`` from the state/vault schema plus explicit carry, ``ParamTree``
  from the components' ``params`` declarations.
- :mod:`icon_sc.core.functional.compile` — composition → pure
  ``step_fn(StateTree, ParamTree, StaticArgs) -> StateTree`` and the
  ``scan_window`` (``lax.scan`` + per-step ``jax.checkpoint``) window builder.
- :mod:`icon_sc.core.functional.rules` — implicit-function-theorem helpers for
...
from icon_sc.core.functional.compile import (
...
from icon_sc.core.functional.pytree import (
    build_param_tree,
    build_state_tree,
    make_pytree_type,
    mapping_of,
